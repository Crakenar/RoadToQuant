...
int main(int argc, char** argv)
{
    ...
    cv::CommandLineParser parser(argc, argv, "{@input | best.onnx | input image}");
    auto path = parser.get<cv::String>("@input");

    cv::dnn::Net net = Helper::readFromOnnx(path);
    cv::Mat imgToModify = Helper::readAndOpenImageCustom("test.jpg");
    ...
    auto detections = DnnHelper::runInference(net, imgToModify, inputSize, numClasses, confThreshold, nmsThreshold);
    ...
    // Take the highest-confidence detection as "the" leaf to crop
    const auto best = ranges::max_element(detections,
                                    [](const DnnHelper::Detection& a, const DnnHelper::Detection& b) { return a.confidence < b.confidence; });

    // Clamp box to image bounds (predicted box can slightly overshoot edges)
    cv::Rect box = best->box & cv::Rect(0, 0, imgToModify.cols, imgToModify.rows);

    cv::Mat cropped = imgToModify(box);
