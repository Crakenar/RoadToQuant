...
export default function StatsScreen() {
  ...
  const streak = (() => {
    ...
    while (true) {
      ...
      if ((reviewHistory[key] ?? 0) > 0) { count++; d.setDate(d.getDate() - 1); }
      else break;
    }
    return count;
  })();
  ...
  for (const q of questions) {
    const card = srs[q.id];
    if (!card || card.dueDate === null) newCards++;
    else if (card.repetitions < 3 || card.interval <= 1) learning++;
  ...
  const hardest = questions
    ...
    .map((x) => ({
      q: x.q,
      accuracy: Math.round((x.s!.correct / x.s!.attempts) * 100),
                ...
                <View style={styles.forecastBarTrack}>
                  <View
                    style={[
                      ...
                      {
                        height: `${Math.round((count / maxForecast) * 100)}%` as any,
          ...
          {recentWrongQuestions.length === 0 ? (
            ...
            <>
                ...
                return (
                  <View key={q.id} style={styles.mistakeCard}>
                    ...
                    <TouchableOpacity
                      ...
                      activeOpacity={0.8}
                    >
                      <Text style={styles.retryBtnText}>Retry this card</Text>
                    </TouchableOpacity>
                  </View>
                );
              })}
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate('Quiz', {
                  categoryId: 'all',
                  categoryName: 'Recent mistakes',
                  mode: 'quiz' as const,
                  questionIds: recentWrongQuestions.map((q) => q.id),
                })}
                activeOpacity={0.85}
              >
                <Text style={styles.reviewButtonText}>Review all mistakes ({recentWrongQuestions.length})</Text>
              </TouchableOpacity>
            </>
          )}
        </View>
          ...
          {hardest.length === 0 ? (
            ...
            <>
              ...
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate('Quiz', {
                  categoryId: 'all',
                  categoryName: 'Hardest cards',
                  mode: 'quiz' as const,
                  questionIds: hardest.map(({ q }) => q.id),
                })}
                activeOpacity={0.85}
              >
                <Text style={styles.reviewButtonText}>Review these {hardest.length} questions</Text>
              </TouchableOpacity>
            </>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  todayEmoji: { fontSize: 18, marginBottom: 4 },
  ...
  stateChip: { flex: 1, borderRadius: 12, paddingVertical: 12, alignItems: 'center' },
  ...
  correctAnswerRow: { flexDirection: 'row', flexWrap: 'wrap', marginBottom: 8 },
  ...
  resetButton: {
    ...
    alignItems: 'center', borderWidth: 2, borderColor: '#FEE2E2', marginBottom: 8,
  },
